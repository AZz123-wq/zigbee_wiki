---
title: "Binary Input (BACnet Extended) Cluster"
type: entity
sources:
...
# Binary Input (BACnet Extended) Cluster (0x0609)

## 概述

Binary Input (BACnet Extended) Cluster（Cluster ID `0x0609`）属于 ZCL Rev 7 的 Protocol Interfaces 功能域，章节定位为 9.2-9.4。Binary Input (BACnet Extended) 是 Protocol Interfaces 章节中的协议隧道或 BACnet 风格 I/O/value cluster。

Rev 7 精读范围：`raw/specs/07-5123-07-ZigbeeClusterLibrary_Revision_7-1.pdf p.547-p.549`。本页根据这些页的 cluster identifier、overview、server/client、attributes、commands 和行为说明整理；长表字段仍以来源页为准。
...
|------|------|
| Cluster ID | `0x0609` |
| 名称 | Binary Input (BACnet Extended) |
...
Binary Input (BACnet Extended) 是 Protocol Interfaces 章节中的协议隧道或 BACnet 风格 I/O/value cluster。
...
- `0x0609` 的权威定义来自 ZCL Rev 7；测试规范是 certification 要求，不能直接替代协议行为。
