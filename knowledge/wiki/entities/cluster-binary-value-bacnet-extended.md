---
title: "Binary Value (BACnet Extended) Cluster"
type: entity
sources:
...
# Binary Value (BACnet Extended) Cluster (0x060D)

## 概述

Binary Value (BACnet Extended) Cluster（Cluster ID `0x060D`）属于 ZCL Rev 7 的 Protocol Interfaces 功能域，章节定位为 9.2-9.4。Binary Value (BACnet Extended) 是 Protocol Interfaces 章节中的协议隧道或 BACnet 风格 I/O/value cluster。

Rev 7 精读范围：`raw/specs/07-5123-07-ZigbeeClusterLibrary_Revision_7-1.pdf p.552-p.554`。本页根据这些页的 cluster identifier、overview、server/client、attributes、commands 和行为说明整理；长表字段仍以来源页为准。
...
|------|------|
| Cluster ID | `0x060D` |
| 名称 | Binary Value (BACnet Extended) |
...
Binary Value (BACnet Extended) 是 Protocol Interfaces 章节中的协议隧道或 BACnet 风格 I/O/value cluster。
...
- `0x060D` 的权威定义来自 ZCL Rev 7；测试规范是 certification 要求，不能直接替代协议行为。
