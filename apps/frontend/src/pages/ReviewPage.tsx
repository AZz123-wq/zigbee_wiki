          ...
          <div className="space-y-3">
            {items.map((item) => {
              ...
              return (
                <div
                  key={item.id}
                  className="min-w-0 bg-gray-800/50 border border-gray-700 rounded-lg p-3 sm:p-4 hover:border-gray-600 transition-colors"
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm mt-1.5 break-words">{item.title}</p>
                      <p className="text-xs text-gray-400 mt-1 break-words">{item.summary}</p>
                      <div className="flex flex-wrap gap-1 mt-2">
                        {item.related_raw_files?.map((f: string, i: number) => (
                          <span key={i} className="inline-flex max-w-full min-w-0 text-[10px] px-1.5 py-0.5 rounded bg-blue-600/10 text-blue-300">
